...
public class JsonTableProducerTest {
    ...
    public void testProduce() throws Exception {
        ...
        Assert.assertEquals(tables.get(0).getRows().get(0).getValues().toString(), "{id=1, firstname=Sheldon, lastname=Cooper, username=flash, password=%$&&%$}");
        Assert.assertEquals(tables.get(0).getRows().get(1).getValues().toString(), "{id=2, firstname=Leonard, lastname=Hofstadter, username=batman, password=secr3t, email=leo@bigbangtheory.org}");
