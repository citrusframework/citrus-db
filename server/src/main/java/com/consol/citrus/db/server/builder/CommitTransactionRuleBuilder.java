...
public class CommitTransactionRuleBuilder extends AbstractDecisionMakingRuleBuilder<CommitTransactionRule, Void>{

    CommitTransactionRuleBuilder(final RuleBasedController controller) {
        super(controller);
    }

    @Override
    protected CommitTransactionRule createRule(
            final Precondition<Void> precondition,
            final Mapping<Void, Boolean> mapping) {
        return new CommitTransactionRule(precondition, mapping);
    }
}
